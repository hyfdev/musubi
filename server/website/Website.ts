...
import { MusubiPage, type MusubiPageData } from '../musubi-notion/MusubiPage'
...
export class Website {
  ...
  static async getInstance(): Promise<Website> {
    if (!Website.instance) {
      ...
      logger.info(`[Website] getInstance: useSnapshot=${shouldUseSnapshot()}`)
      if (shouldUseSnapshot()) {
        ...
        databasePageId = import.meta.env.VITE_NOTION_DATABASE_PAGE_ID || ''
        if (!databasePageId) {
          throw new Error('VITE_NOTION_DATABASE_PAGE_ID environment variable is not set')
        }
      }
