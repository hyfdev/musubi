...
`Musubi CJK Fallback` is derived from [`LXGW WenKai GB`](https://github.com/lxgw/LxgwWenKaiGB) Medium release `v1.522`, asset `LXGWWenKaiGB-Medium.ttf`, SHA-256 `b885c51ec0d3f325974013801dfcefda1a9ba0bf385c607cf5f2582dafa2e5ab`. The repository carries eight verified, renamed runtime WOFF2 shards under `scripts/font/prebuilt-fallback/`. They contain every mapping from that pinned source that Musubi's Chinese typography classifier can select and exclude ASCII, ordinary Latin, and every unrelated mapping. Ordinary builds copy all eight content-addressed shards into the deployment after Tsanger; exact `unicode-range` declarations let the browser request one only when a rendered Chinese character is absent from the preferred Tsanger subset, including Chinese introduced after the build.
