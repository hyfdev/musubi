...
`Musubi CJK Fallback` is derived from [`LXGW WenKai GB`](https://github.com/lxgw/LxgwWenKaiGB) Medium release `v1.522`, asset `LXGWWenKaiGB-Medium.ttf`, SHA-256 `b885c51ec0d3f325974013801dfcefda1a9ba0bf385c607cf5f2582dafa2e5ab`. The repository carries 32 verified, renamed runtime WOFF2 shards under `scripts/font/prebuilt-fallback/`: ten fixed common-character shards and 22 region-bounded shards for the remaining mappings. They contain every mapping from that pinned source that Musubi's Chinese typography classifier can select, exclude ASCII, ordinary Latin, and every unrelated mapping, and stay below the reviewed 600,000-byte per-shard ceiling. Ordinary builds copy all 32 content-addressed shards into the deployment after Tsanger; exact `unicode-range` declarations let the browser request one only when a rendered Chinese character is absent from the preferred Tsanger subset, including Chinese introduced after the build. The common-character data is a checked-in, checksummed snapshot with its source provenance; ordinary builds do not contact Google Fonts.
...
Generated runtime shards use the non-reserved `Musubi CJK Fallback` identity, retain copyright and OFL metadata, ship SIL Open Font License 1.1, and are validated before publication. A required build-time Chinese typography mapping absent from the verified LXGW pool fails generation rather than silently switching to an unrelated system face.