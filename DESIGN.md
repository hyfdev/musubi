...
- **X post embeds:** Treat the official X widget as an optional enhancement rather than the only readable representation. Generated HTML must contain a complete static quotation with author, handle, post text, publication label, and an external X link whenever build-time enrichment succeeds; render that fallback on the page canvas with one quiet primary border, a restrained `6px` radius, and the existing text hierarchy. Replace it only after the first official widget renders successfully. When the resolved Light or Dark theme changes, keep the current widget visible while a same-width hidden replacement renders, then swap them directly without showing the fallback, collapsing the component, or adding a decorative transition. If that theme rebuild fails, retain the current widget. If initial browser enhancement fails, keep the complete quotation; if build-time enrichment fails, keep an ordinary safe link. The bounded oEmbed request may obtain the quotation, but it is the only X-related network request allowed during generation and must not be used to derive widget height. Generation must not make any network request or launch a local or remote browser to calculate height; any future height reservation must use deterministic local information and remain optional. Never render provider-returned HTML directly or introduce X-specific colors, shadows, or a competing card language.
