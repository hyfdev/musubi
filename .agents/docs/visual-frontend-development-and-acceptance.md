...
- Inspect navigation with visible, hidden, ordered, and missing-order Page rows when those states are affected. For an affected X embed, verify direct entry and client-side navigation, Light and Dark recreation, the official widget success state, the complete static quotation when browser enhancement is blocked, and the ordinary safe link when build-time enrichment is unavailable. No X script or request should appear on a route without an enriched X embed.
