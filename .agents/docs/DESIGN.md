...
Typography carries most of Musubi's identity. Chinese body and ordinary interface text use Tsanger JinKai 02 W04; Chinese headings, table headers, and explicit emphasis use W05. English headings, prose, metadata, and interface text use Charter with conventional serif fallbacks. Code uses JetBrains Mono, with W04 available for Chinese code text. Do not introduce a default sans-serif interface family.

Generate W04 and W05 from one shared build-known Chinese typography corpus covering the complete static Site, including the optional Home, site identity, navigation, metadata, interface strings, prose, headings, emphasis, tables, and Chinese code. Both subsets request every collected Chinese code point even though components continue to select W04 or W05 by their visual role. Assign punctuation shared by Chinese and Latin according to the neighboring script within the same text run, using the same classification during rendering and font generation. Inline emphasis, deletion, and link markup do not split a run; a visual line break or inline-code boundary ends it. Publish the complete `Musubi CJK Fallback` runtime pool derived from pinned `LXGW WenKai GB Medium v1.522` after the Tsanger faces. Its 32 content-addressed shards use ten fixed common-character groups and 22 region-bounded cold groups, cover every LXGW mapping the Chinese typography classifier can select, including Chinese introduced after the static subsets were built, and exclude ASCII, ordinary Latin, and all other mappings that cannot enter Chinese typography. Let `unicode-range` keep unused shards off the network. Do not synthesize missing weights or styles.

Self-host content-derived Charter regular, italic, bold, and bold italic subsets for English, and the same four style combinations from JetBrains Mono for code. The selected faces, generated font stylesheet, and WOFF2 files use pinned inputs and content-addressed immutable URLs; all faces use `font-display: swap`. Do not preload every face globally because pages use different combinations.
...
- **Tables:** Use heading type for header text, including W05 for Chinese, together with a light neutral header, the primary divider below a header row, and secondary horizontal separators between ordinary rows. A table wider than the reading column scrolls in its own container; it never widens or scrolls the page.
