...
- **Fallback delivery:** Pin `LXGW WenKai GB Medium v1.522` as the source for `Musubi CJK Fallback`. Generate eight checked-in WOFF2 shards containing every mapping from that source that Musubi's Chinese typography classifier can ever select: Han characters, CJK punctuation, full-width forms, and punctuation that can follow Chinese context. Exclude ASCII, ordinary Latin, and every other mapping that cannot enter Chinese typography. Publish all eight shards with every site build, after Tsanger in the family stacks; `unicode-range` lets the browser fetch a shard only when the preferred Tsanger subset does not cover a rendered character. Use the same Medium outlines for weights 400 through 500 rather than synthesizing another weight or introducing a second fallback typeface.
...
- **Regeneration, caching, and validation:** Every static build derives or validates its content subsets against the complete Site, source fonts, pinned tools, and complete runtime fallback. Generated WOFF2 files and the generated font stylesheet use content-addressed URLs with immutable caching. Both Tsanger subsets must be requested from the same Chinese corpus; a requested mapping supported by a source must survive its subset. Every build-known Chinese mapping must exist in the published LXGW runtime pool even when Tsanger also supplies it, and absence from that pool stops the build. The full quality gate decodes every checked-in shard, verifies exact non-overlapping coverage, and rejects mappings that the Chinese classifier can never select.
