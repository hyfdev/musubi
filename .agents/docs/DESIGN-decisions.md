...
### Chinese font delivery and build-time fallback

- **Date:** 2026-07-22.
- **Status:** Required.
- **Scope:** Generated Tsanger JinKai webfont artifacts for Musubi's static output and LXGW WenKai GB glyphs used when a build-known Chinese character is absent from either selected Tsanger source. The original Tsanger TTF files remain private build inputs.
...
- **Fallback delivery:** Pin `LXGW WenKai GB Medium v1.522` as the source for `Musubi CJK Fallback`. Keep the checked-in verified full-coverage shards as a build source pool, but publish only content-derived WOFF2 subsets for requested Chinese typography code points absent from either Tsanger source. A generated fallback subset must not expose ASCII or unrelated Latin mappings. Use the same Medium outlines after W04 and W05 rather than synthesizing another weight or introducing a second fallback typeface.
- **Runtime boundary:** Current Musubi output is static and has no unbounded browser-time Chinese source, so a character that first appears after deployment falls to the system fallback rather than causing every deployment to publish complete LXGW coverage. A future feature such as comments, user input, or client-fetched content must explicitly reopen this boundary and add measured runtime coverage suitable for its actual character set.
- **Regeneration, caching, and validation:** Every static build derives or validates its subsets against the complete Site, source fonts, and pinned tools. Generated WOFF2 files and the generated font stylesheet use content-addressed URLs with immutable caching. Both Tsanger subsets must be requested from the same Chinese corpus; a requested mapping supported by a source must survive its subset. Every requested mapping missing from either Tsanger source must survive in an LXGW subset, and absence from all applicable sources stops the build. The complete checked-in LXGW source pool remains separately decoded and verified by the full quality gate.
...
- **Reason or reference:** Yunfei explicitly chose the shared W04/W05 corpus on 2026-07-22 because the small duplicated subset cost is preferable to making delivery correctness depend on every visual role and corpus role staying synchronized. The review that prompted this change found a real table header where two missing W05 mappings caused two coarse LXGW shards totaling about `2.56 MiB` on a cold cache; it also found that the old full fallback exposed ASCII before the generic serif and monospace families. The current static content has no need for the previous complete runtime bundle. LXGW Medium remains the selected missing-glyph source because its Simplified Chinese forms and density are compatible with the accepted Tsanger pairing.
