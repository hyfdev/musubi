...
- **Limits:** The build generates the Git-ignored root `wrangler.json`, where Wrangler discovers it directly, and the configuration names the connected `hyf-me` Worker and selects only `dist/client` for deployment. That name is explicit deployment metadata, not a value inferred from the package or application name; renaming or moving the Worker requires updating the generated contract. The required compatibility date and the explicit 404 and URL handling remain generated. The Worker has no `main`, assets binding, runtime Notion credentials, or server process. `dist/ssr` remains a local build artifact and may remain on disk.
- **Why:** Yunfei rejected Pages because it is being retired in favor of Workers, selected Workers explicitly, required the Void migration to keep Cloudflare deployment, and wants the Wrangler configuration to be a build result rather than independently maintained source. Cloudflare PR #1 proved that the previous package-derived `musubi` name did not identify the custom deployment target. A repository build cannot discover that arbitrary identity, while omitting it still triggers Wrangler's connected-build name-conflict path, so the generated contract must state the actual Worker name. Wrangler routing remains necessary for a visible 404 and slashless canonical URLs.
