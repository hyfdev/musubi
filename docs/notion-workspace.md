...
`pnpm run dev`, `vp run site:build`, and `vp run ready` read only this local snapshot. `pnpm run build` runs `notion:setup` first and then `site:build`. The complete deployable site is `dist/client`; it contains neither the snapshot nor Notion credentials.
