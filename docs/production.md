...
The default cloud build verifies and copies the checked-in, open-licensed LXGW fallback WOFF2 shards instead of downloading the 25 MB source TTF and regenerating the same complete coverage. The repository retains the source version, checksums, renamed identity, OFL notice, and a maintenance-only rebuild program. `vp run font:setup` runs from `postinstall`, `dev`, and `site:build`; a missing cache downloads the pinned Tsanger sources and must succeed, or the pipeline stops with an error. Set `MUSUBI_TSANGER_W04_URL` / `MUSUBI_TSANGER_W05_URL` in the builder to mirror sources, or `MUSUBI_TSANGER_SETUP=0` to skip the download attempt (clear the private cache if the builder must not use a prior Tsanger pair). Raw Tsanger files and the ignored `.musubi/font/` cache must never become deployment artifacts.
